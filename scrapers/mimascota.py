...
def main(base_url: str, page_notation: str, category: str):
    ...
    while True:
        ...
        lis: ResultSet[Tag] = soup.find_all("li", attrs={"class": "prod__box"})
        if not lis:  # Stop if no more products are found
            break

        # Extract and write product data to the Excel sheet
        for li in lis:
            nombre_tag = li.find("h3", attrs={"class": "prod__box--prod-name--title"})
            precio_tag = li.find("div", attrs={"class": "prod__box--price"})
