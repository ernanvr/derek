...
def main(base_url: str, page_notation: str, category: str):
    ...
    while True:
        ...
        lis: ResultSet[Tag] = soup.find_all("li", attrs={"class": "item-producto"})
        if not lis:  # Stop if no more products are found
            break

        # Extract and write product data to the Excel sheet
        for li in lis:
            nombre_tag = li.find("h5", attrs={"class": "prod-nombre"})
            precio_tag = li.find("strong", attrs={"class": "precio"})
    ...
    file_path = os.path.join(current_path, "products_superselectos.xlsx")
